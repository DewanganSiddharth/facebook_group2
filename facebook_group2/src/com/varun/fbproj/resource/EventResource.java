...
public class EventResource {
	...
	@POST
	@Path("/lab1") // event is updated
	@Consumes({MediaType.TEXT_PLAIN})
	@Produces({MediaType.APPLICATION_JSON})
	public ArrayList<Event> updateEvent2(@CookieParam("ID") String jwt,String femailid) throws    UnsupportedEncodingException{
	Claims claims = Jwts.parser()         
			       .setSigningKey("secret".getBytes("UTF-8"))
			       .parseClaimsJws(jwt).getBody();
			  String myEmailID=claims.getSubject();
			  //  user id from user mail id
			  System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++my email id is: "+myEmailID+" friend email id is: "+femailid);
			  int luid=RetriveService.uidfromEmailID(myEmailID);
			  int fuid=RetriveService.uidfromEmailID(femailid);
			  ArrayList<Event> eal= new ArrayList<Event>();
			  eal=EventService.searchnecessaryevents(luid,fuid);
	...
	@POST
	@Path("/labtest") // event is updated
	@Consumes({MediaType.TEXT_PLAIN})
	@Produces({MediaType.APPLICATION_JSON})
	public ArrayList<Event> updateEvent1(@CookieParam("ID") String jwt,String femail) throws    UnsupportedEncodingException{
	Claims claims = Jwts.parser()         
			       .setSigningKey("secret".getBytes("UTF-8"))
			       .parseClaimsJws(jwt).getBody();
			  String myEmailID=claims.getSubject();
			  //  user id from user mail id
			  System.out.println("my email id is: "+myEmailID+" friend email id is: "+femail);
			  int luid=RetriveService.uidfromEmailID(myEmailID);
			  int fuid=RetriveService.uidfromEmailID(femail);
			  ArrayList<Event> eal= new ArrayList<Event>();
			  eal=EventService.searchnecessary(luid,fuid);
	...
	@POST
	@Path("/labtest2") // event is updated
	@Consumes({MediaType.APPLICATION_JSON})
	@Produces({MediaType.APPLICATION_JSON})
	public ArrayList<User> updateEvent2(@CookieParam("ID") String jwt,Event eobj) throws    UnsupportedEncodingException{
		Claims claims = Jwts.parser()         
			       .setSigningKey("secret".getBytes("UTF-8"))
			       .parseClaimsJws(jwt).getBody();
			  String myEmailID=claims.getSubject();	  
		...
		ArrayList<User> u= new ArrayList<User>();
			  ...
			  u=EventService.getfriends(myEmailID,eid);
