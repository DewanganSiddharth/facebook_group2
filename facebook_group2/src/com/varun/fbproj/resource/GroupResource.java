...
return user_list;
   ...
   @GET
   @Path("/getGroupMembers1")
   @Produces({MediaType.APPLICATION_JSON})
      public ArrayList<User> getMembersOfGroup1(@CookieParam("ID_group") String group_name) throws JsonParseException, JsonMappingException, IOException
      {
       String gname=group_name.replaceAll("%20", " ");
       System.out.println("Inside getGroupMembers ");
       ArrayList<User> user_list= new ArrayList<User>(); 
   ...
   String emailid=uobj.getEmailID();
       
       
       for(int i=0;i<user_list.size();i++)
       {
       if((emailid.equals(user_list.get(i).getEmailID())))
       {
       user_list.remove(i);
       }
